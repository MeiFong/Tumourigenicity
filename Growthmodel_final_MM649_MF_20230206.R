...
setwd("L:/Joint_Projects/Analysis_MF/Exponential model_Tumor.Growth")
...
lin.exp <- lm(log(value+1) ~ time, data = df_long_649_t13)
# back-transform parameters: exp(intercept) = a, beta = k
exp.start= list(a= exp(coef(lin.exp))[1], k =coef(lin.exp)[2])

# # Gomperz : 
# xydata <- sortedXyData("time","value",df_long_649_t13) # provides the average y for each unique value of x 
# x = xydata["x"]
# y = xydata["y"]
# # Grid search - Gomperz
...
# fixed effect of k
model_expo <- 
  nlme(value ~f.exp(time, a, k), data = df_long_649_t13, 
       #random = a+k ~ 1|ID,
       ...
       control = list (msMaxIter = 100000, msMaxEval = 1000),  #
       verbose = TRUE
  )
...
# model_gomp3 <- 
#   nlme(value ~ f.gomp3(time, b,d,e), data = df_long_649_t13, 
#       #  random = b +d ~ 1|ID,
...
# 
# model_logistic3 <- 
#   nlme(value ~ f.logit3(time, b,d,e), data = df_long_649_t13, 
#        #random = a+k ~ 1|ID,
#        random = list(ID = pdDiag(d ~ 1)),
#        fixed = list(b ~ Groups,    
#                     d ~ Groups,
...
margin.k <- emmeans(model_expo,  ~ Treat*Groups, data = df_long_649_t13)
contrast.k <- contrast(margin.k, method= "revpairwise", infer =T, adjust = "none") %>% as.data.frame()
...
# compapre initial volume at time 13:
# df_long_649_t13 %>%
#   filter(time ==0) %>%
#   ggplot(aes(y= value,x = Groups )) +
#   geom_boxplot() 
# 
# lm(value ~ Groups, data = df_long_649_t13 %>% filter(time ==0)) %>% broom::tidy()
...
df_fitted.ID <- cbind(df_long_649_t13, df_pred.expo)

# fitted line by Groups
df_groups <- df_long_649_t13 %>% dplyr::select(Groups, time) %>% distinct() 

df_fitted.groups <-  cbind(df_long_649_t13 %>% dplyr::select("Groups", "Group", "Treat", "time") %>% distinct(),
...
###########################################
# Save results for MM649
path = "L:/Lab_Stats_Unit/Satomi/QIMR/MeiFong/tumour growth model/Exponential model_Growth.model/"
write.csv(df_margin.k, file = paste0(path, "parameter.k_Expomodel_649", ".csv" ), row.names = F)
write.csv(df_GR.r, file = paste0(path, "GR.perday_Expomodel_649", ".csv" ), row.names = F)
write.csv(df_Doubling.time, file = paste0(path, "Doubling.time_Expomodel_649", ".csv" ), row.names = F)

write.csv(contrast.k, file = paste0(path, "Contrast_GR_Expomodel_649", ".csv" ), row.names = F)


#dataset for plots
write.csv(df_fitted.ID, file = paste0(path, "df.input.predicted.id_Expomodel_649", ".csv" ), row.names = F)
write.csv(df_fitted.groups, file = paste0(path, "df.predicted.groups_Expomodel_649", ".csv" ), row.names = F)
...
ggsave(paste0(path, "lineplot_fitted_Expomodel_649", ".png"),
...
####################
rm(list=ls())
setwd("L:/Joint_Projects/Analysis_MF/Exponential model_Tumor.Growth/MM649")

# input dataset and fitted data from exponential model ("predict.ID")
df_fitted.ID.649 <- read.csv("df.input.predicted.id_Expomodel_649.csv")
# predicted line for each group
df_fitted.groups.649 <- read.csv("df.predicted.groups_Expomodel_649.csv")
...
lplot_pred.expo.649 <-
  ...
  geom_point(aes(y=value , x=time, group = ID, color =as.factor(Mouse)),
             alpha = 0.5, size =1) +
  geom_line(data = df_fitted.groups.649, aes(y=predicted.groups, x= time, group = Groups),
            size =1, color ="red") +
  facet_grid(Group ~ Treat) +
  scale_y_continuous(limits=c(0,1000)) +
  theme_bw() +
